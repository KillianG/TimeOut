...
public class SpawnsManager {
	...
	public SpawnsManager(TimeOut plugin) {
		this.p = plugin;
	...
	public void addSpawnPoint(final Location location) {
		...
		if(!(spawnPoint.getWorld().getEnvironment() == Environment.NETHER)) {
			spawnPoint.setY(location.getWorld().getHighestBlockYAt(location.getBlockX(), location.getBlockZ()));
		}
		else {
			Location safeSpot = Util.searchSafeSpot(location);
			if(safeSpot == null) {
		...
		if(Util.isInsideBorder(spawnPoint, Bukkit.getOnlinePlayers().size() * 100)) {
			...
			for(Object position : p.getConfig().getList("spawnpoints")) {
				if(position instanceof String && position != null) {
					...
					try {
						...
						p.getLogger().info("load.spawnPointAdded" + coords[0] + ";" +  coords[1]);
						spawnCount++;
					} catch(Exception e) { // Not an integer or not enough coords
						p.getLogger().warning("load.invalidSpawnPoint" +  (String) position);
					}
				}
			}
	...
	public boolean generateRandomSpawnPoints(World world, int spawnCount, int regionDiameter, int minimalDistanceBetweenTwoPoints, double xCenter, double zCenter) {
		...
		generationLoop: while(generatedSpawnPoints != spawnCount) {
			...
			// Inside the region?
			if(Util.isInsideBorder(randomPoint, regionDiameter)) {
			...
			// Safe spot available?
			if(!Util.isSafeSpot(surfaceBlock.getLocation())) {
...
//		
		...
		addOnSide[0] = new Location(world, -distanceBetweenTwoPoints, 100, 0); // North side, direction east
		addOnSide[1] = new Location(world, 0, 100, distanceBetweenTwoPoints);  // East side,  direction south
		addOnSide[2] = new Location(world, distanceBetweenTwoPoints, 100, 0);  // South side, direction west
		addOnSide[3] = new Location(world, 0, 100, -distanceBetweenTwoPoints); // West side,  direction north
		...
		generationLoop: while(true) {
			...
			// First point
			if(Util.searchSafeSpot(currentPoint) != null) {
			...
			for(int j = 0; j < 4; j++) { // A step for each side, j is the side (see addOnSide).
				...
				sideLoop: while(plottedSize < currentSquareSize) {
					...
					// Inside the border?
					if(Util.isInsideBorder(currentPoint, regionDiameter)) {
						continue sideLoop;
					}
	...
	public boolean generateCircularSpawnPoints(World world, int spawnCount, int regionDiameter, int minimalDistanceBetweenTwoPoints, double xCenter, double zCenter) {
		...
		generationLoop: while(currentCircleDiameter >= minimalDistanceBetweenTwoPoints) {
			...
			circleLoop: while(currentAngle <= 2 * Math.PI - angleBetweenTwoPoints + startAngle) {
				...
				if(Util.isInsideBorder(point, regionDiameter)) { // Just in case
					continue circleLoop;
				}
